...
async def _legal_zapis(message):
    if not await student_privacy_notice_completed(message.from_id, "vk"):
        ...
        await message.answer(
            ...
            "Для оформления записи Сервис использует необходимые идентификаторы VK и сведения о выбранном занятии. Метка источника перехода используется для оценки каналов привлечения.\n\n"
