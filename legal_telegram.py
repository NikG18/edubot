...
async def _show_privacy_notice(message, context: str, continue_callback: str):
    ...
    await message.answer(
        ...
        "включая ваш Telegram ID/username и сведения о выбранном занятии. Метка источника перехода сохраняется отдельно для оценки эффективности каналов привлечения.\n\n"
...
async def _legal_start_trials_booking(call, state):
    ...
    if not await student_privacy_notice_completed(call.from_user.id, "telegram"):
        ...
        text = (
            ...
            "Для оформления записи Сервис использует необходимые идентификаторы и сведения о выбранном занятии. Метка источника перехода используется для оценки каналов привлечения."
        )
        if call.message.content_type != "text":
            await call.message.delete()
...
async def _legal_buy_subscription_start(call, state):
    ...
    if not await student_privacy_notice_completed(call.from_user.id, "telegram"):
        ...
        await call.message.edit_text(
            ...
            "Для оформления покупки Сервис использует необходимые идентификаторы и сведения о выбранном абонементе. Метка источника перехода используется для оценки каналов привлечения.\n\n"
