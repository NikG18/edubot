...
from vkbottle import (
    ...
    BaseStateGroup, BuiltinStateDispenser, Callback
)
...
# -------------------- Вспомогательные функции для работы с callback-событиями --------------------
async def edit_event_message(event: MessageEvent, text: str, keyboard: str = None):
    """Редактирует сообщение, вызвавшее событие."""
...
async def answer_event(event: MessageEvent, text: str = None, snackbar: bool = True):
    """Подтверждает событие и, при необходимости, показывает уведомление."""
    event_data = None
    if text and snackbar:
        event_data = {"type": "show_snackbar", "text": text}
    elif text:
        event_data = {"type": "show_snackbar", "text": text}  # по умолчанию показываем уведомление
...
async def make_tutors_keyboard(callback_prefix: str, back_callback: str = "back_to_menu") -> str:
    ...
    for tid, tdata in tutors.items():
        kb.add(Callback(tdata["name"], payload={"cmd": f"{callback_prefix}_{tid}"}))
        kb.row()
    kb.add(Callback("🔙 Назад в меню", payload={"cmd": back_callback}))
    return kb.get_json()
...
async def make_subjects_keyboard(tutor_id: int, back_callback: str = "back_to_menu") -> str:
    ...
    if not tutor:
        kb = Keyboard(inline=True)
        kb.add(Callback("🔙 Назад", payload={"cmd": back_callback}))
        return kb.get_json()
    kb = Keyboard(inline=True)
    for subj in tutor["subjects"]:
        kb.add(Callback(subj, payload={"cmd": f"subject_{tutor_id}_{subj}"}))
        kb.row()
    kb.add(Callback("🔙 Назад к репетиторам", payload={"cmd": back_callback}))
    return kb.get_json()
...
async def create_and_send_payment(source, booking, email, booking_id):
    ...
    if not payment_url:
        await bot.api.messages.send(user_id=booking["user_id"],
                                    message="Ошибка создания платежа. Обратитесь в поддержку.", )
        return
...
async def show_tutor_info(event: MessageEvent):
    ...
    keyboard.add(Callback("🎓 Записаться на пробное занятие", payload={"cmd": "trials", "tutor_id": tid}))
    keyboard.row()
    keyboard.add(Callback("🔙 Назад к списку", payload={"cmd": "back_to_tutors"}))
    await edit_event_message(event, text, keyboard=keyboard.get_json())
...
async def start_trials_booking(event: MessageEvent):
    ...
    for subj in subjects:
        keyboard.add(Callback(subj, payload={"cmd": "trial_subject", "subject": subj}))
        keyboard.row()
...
async def show_trial_dates(event: MessageEvent, tid: int):
    ...
    for d in available_dates:
        ...
        buttons.append(Callback(label, payload={"cmd": "trial_date", "date": d}))
    kb = Keyboard(inline=True)
    for btn in buttons:
        kb.add(btn)
        kb.row()
...
async def trial_date_chosen(event: MessageEvent):
    ...
    if not slots:
        kb = Keyboard(inline=True)
        kb.add(Callback("🔙 К выбору даты", payload={"cmd": "back_to_trial_dates"}))
        await edit_event_message(event, "На эту дату нет свободного времени.", keyboard=kb.get_json())
        return

    kb = Keyboard(inline=True)
    for s in slots:
        kb.add(Callback(s, payload={"cmd": "trial_slot", "slot": s}))
        kb.row()
    kb.add(Callback("🔙 К выбору даты", payload={"cmd": "back_to_trial_dates"}))
...
async def trial_slot_chosen(event: MessageEvent):
    ...
    keyboard = Keyboard(inline=True)
    keyboard.add(Callback("✅ Подтвердить", payload={"cmd": "confirm_trial"}))
    keyboard.row()
    keyboard.add(Callback("🔙 К выбору времени", payload={"cmd": "back_to_trial_dates"}))
    await edit_event_message(event, text, keyboard=keyboard.get_json())
...
async def lesson_info(message: Message):
    ...
    keyboard = Keyboard(inline=True)
    keyboard.add(Callback("🔙 Назад в меню", payload={"cmd": "back_to_menu"}))
...
async def zapis(message: Message):
    await message.answer("Кто из репетиторов вас интересует?",
...
async def subject_chosen(event: MessageEvent):
    ...
    if not dates:
        ...
        await edit_event_message(event,
                                 "У этого преподавателя пока нет свободных дат. Попробуйте позже или свяжитесь с преподавателем.",
    ...
    for d in dates:
        dt = datetime.strptime(d, "%d.%m.%Y")
        label = f"{d} ({WEEKDAY_NAMES[WEEKDAYS[dt.weekday()]]})"
        kb.add(Callback(label, payload={"cmd": f"date_{d}"}))
        kb.row()
    kb.add(Callback("🔙 Назад к репетиторам", payload={"cmd": "back_to_tutors_booking"}))
...
async def choose_date(event: MessageEvent):
    ...
    kb = Keyboard(inline=True)
    for s in slots:
        kb.add(Callback(s, payload={"cmd": f"slot_{s}"}))
        kb.row()
    kb.add(Callback("🔙 К выбору даты", payload={"cmd": "back_to_date"}))
    await edit_event_message(event, "Выберите время:", keyboard=kb.get_json())
...
async def choose_slot(event: MessageEvent):
    ...
    if "tutor_name" not in data and tid:
        ...
        if tutor:
            ...
            await state_dispenser.update(event.user_id, tutor_name=tutor["name"])
    ...
    text = (f"Проверьте данные:\n"
            ...
            f"Время: {slot}\n\nВсё верно?")
    kb = Keyboard(inline=True)
    kb.add(Callback("✅ Подтвердить запись", payload={"cmd": "confirm_booking"}))
    kb.row()
    kb.add(Callback("✏️ Изменить время", payload={"cmd": "back_to_date"}))
    kb.row()
    kb.add(Callback("❌ Отменить запись", payload={"cmd": "cancel_booking"}))
...
async def cancel_booking(event: MessageEvent):
    await edit_event_message(event, "Запись отменена. Возвращаемся в главное меню.")
    await state_dispenser.delete(event.user_id)
    await bot.api.messages.send(
        user_id=event.user_id,
        message="Главное меню",
...
async def my_records(message: Message):
    ...
    for bid, b in user_bookings:
        ...
        if can_act:
            kb.add(Callback(f"🔄 Перенести: {tutor['name']} {b['date']} {b['time_slot']}",
                            payload={"cmd": f"reschedule_student_{bid}"}))
            kb.row()
            kb.add(Callback(f"❌ Отменить: {tutor['name']} {b['date']} {b['time_slot']}",
...
async def cancel_student_booking(event: MessageEvent):
    ...
    await bot.api.messages.send(user_id=student_id, message="✅ Вы отменили занятие.", )
    if tutor_id and (tutor_tg := tutors.get(tutor_id, {}).get("vk_id")):
        ...
        try:
            await bot.api.messages.send(user_id=tutor_tg, message=msg_tutor, )
        except:
            pass
    kb = Keyboard(inline=True)
...
async def back_to_my_records(event: MessageEvent):
    ...
    if not user_bookings:
        ...
        kb.row()
        kb.add(Callback("🔙 Назад в меню", payload={"cmd": "back_to_menu"}))
        await edit_event_message(event, "У вас пока нет активных записей.", keyboard=kb.get_json())
        return
    text_lines = ["Ваши записи:\n"]
    kb = Keyboard(inline=True)
    tutors = await get_all_tutors()
...
async def student_reschedule_date(event: MessageEvent):
    ...
    if not slots:
        ...
        await edit_event_message(event, "На эту дату нет свободных слотов.", keyboard=kb.get_json())
        return
    kb = Keyboard(inline=True)
    for s in slots:
        kb.add(Callback(s, payload={"cmd": f"reschedule_slot_{s}"}))
        kb.row()
    kb.add(Callback("🔙 К выбору даты", payload={"cmd": "back_to_reschedule_date"}))
...
async def student_reschedule_slot(event: MessageEvent):
    ...
    kb = Keyboard(inline=True)
    kb.add(Callback("✅ Подтвердить перенос", payload={"cmd": "confirm_student_reschedule"}))
    kb.row()
    kb.add(Callback("🔙 Назад к выбору времени", payload={"cmd": "back_to_reschedule_date"}))
    await edit_event_message(event, text, keyboard=kb.get_json())
...
async def confirm_student_reschedule(event: MessageEvent):
    ...
    if tutor_tg:
        ...
        kb.add(Callback("❌ Отклонить", payload={"cmd": f"tutor_reject_{new_id}"}))
        try:
            await bot.api.messages.send(user_id=tutor_tg, message=notify_tutor, keyboard=kb.get_json(), )
        except:
            pass
...
async def oplata(message: Message):
    kb = Keyboard(inline=True)
    kb.add(Callback("📱 Оплата по QR-коду", payload={"cmd": "qr"}))
    kb.row()
    kb.add(Callback("💳 Оплата банковской картой", payload={"cmd": "card"}))
    kb.row()
    kb.add(Callback("📲 Перевод СБП по номеру телефона", payload={"cmd": "sbp"}))
    kb.row()
    kb.add(Callback("🔙 Назад в меню", payload={"cmd": "back_to_menu"}))
...
async def back_to_pay(event: MessageEvent):
    ...
    kb.row()
    kb.add(Callback("🔙 Назад в меню", payload={"cmd": "back_to_menu"}))
    await edit_event_message(event, "Какой способ оплаты вам удобнее?", keyboard=kb.get_json())


async def qr(event: MessageEvent):
    kb = Keyboard(inline=True)
    kb.add(Callback("🔙 Назад к списку", payload={"cmd": "back_to_pay"}))
    await edit_event_message(event, "📱 Сканируйте QR-код для оплаты в приложении вашего банка", keyboard=kb.get_json())


async def card(event: MessageEvent):
    kb = Keyboard(inline=True)
    kb.add(Callback("🔙 Назад к списку", payload={"cmd": "back_to_pay"}))
    await edit_event_message(event, "💳 Переходите по ссылке и следуйте дальнейшим инструкциям", keyboard=kb.get_json())
...
async def material(message: Message):
    kb = Keyboard(inline=True)
    kb.add(Callback("📘 Учебные пособия", payload={"cmd": "book"}))
    kb.row()
    kb.add(Callback("🎥 Авторские видео", payload={"cmd": "vid"}))
    kb.row()
    kb.add(Callback("🔙 Назад в меню", payload={"cmd": "back_to_menu"}))
...
async def back_to_mat(event: MessageEvent):
    ...
    kb.row()
    kb.add(Callback("🔙 Назад в меню", payload={"cmd": "back_to_menu"}))
    await edit_event_message(event, "Вы ищете пособия или видео?", keyboard=kb.get_json())


async def book(event: MessageEvent):
    kb = Keyboard(inline=True)
    kb.add(Callback("🧪 Химия", payload={"cmd": "bookh"}))
    kb.row()
    kb.add(Callback("⚛️ Физика", payload={"cmd": "bookf"}))
    kb.row()
    kb.add(Callback("🔙 Назад к списку", payload={"cmd": "back_to_mat"}))
    await edit_event_message(event, "📘 Учебники и таблицы", keyboard=kb.get_json())


async def vid(event: MessageEvent):
    kb = Keyboard(inline=True)
    kb.add(Callback("🧪 Химия", payload={"cmd": "videh"}))
    kb.row()
    kb.add(Callback("⚛️ Физика", payload={"cmd": "videf"}))
    kb.row()
    kb.add(Callback("🔙 Назад к списку", payload={"cmd": "back_to_mat"}))
    await edit_event_message(event, "🎥 Видеоматериалы (записи реакций и явлений)", keyboard=kb.get_json())


async def bookh(event: MessageEvent):
    await answer_event(event, "Скоро здесь будут пособия по химии", snackbar=True)


async def bookf(event: MessageEvent):
    await answer_event(event, "Скоро здесь будут пособия по физике", snackbar=True)


async def videh(event: MessageEvent):
    await answer_event(event, "Скоро здесь будут видео по химии", snackbar=True)


async def videf(event: MessageEvent):
    await answer_event(event, "Скоро здесь будут видео по физике", snackbar=True)
...
async def choose_msg_tutor(event: MessageEvent):
    ...
    tutor = tutors.get(tid)
    if not tutor:
        await edit_event_message(event, "Преподаватель не найден.")
        return
    await state_dispenser.update(event.user_id, msg_tutor_id=tid, msg_tutor_name=tutor["name"])
    kb = Keyboard(inline=True)
    kb.add(Callback("❌ Отмена", payload={"cmd": "cancel_msg_to_tutor"}))
    await edit_event_message(event, f"Вы пишете преподавателю {tutor['name']}.\nВведите ваше сообщение:",
                             keyboard=kb.get_json())
...
async def cancel_msg_to_tutor(event: MessageEvent):
    ...
    tutors = await get_all_tutors()
    for tid, tdata in tutors.items():
        kb.add(Callback(tdata["name"], payload={"cmd": f"msg_tutor_{tid}"}))
        kb.row()
    kb.add(Callback("🔙 Назад в меню", payload={"cmd": "back_to_menu"}))
    await edit_event_message(event, "Выберите преподавателя, которому хотите написать:", keyboard=kb.get_json())
...
async def send_message_to_tutor(message: Message):
    ...
    try:
        await bot.api.messages.send(user_id=ADMIN_VK_ID, message=forward_msg, keyboard=kb.get_json(), )
    except:
        pass
    tutors = await get_all_tutors()
    tutor = tutors.get(tid)
    if tutor and tutor.get("vk_id"):
        try:
            await bot.api.messages.send(user_id=tutor["vk_id"], message=forward_msg, keyboard=kb.get_json(),
                                        )
        except:
            pass
...
async def send_reply_to_student(message: Message):
    ...
    try:
        await bot.api.messages.send(user_id=student_id, message=reply_text, )
...
async def cancel_tutor_msg_to_student(event: MessageEvent):
    ...
    if tid:
        kb = Keyboard(inline=True)
        kb.add(Callback("📋 Мои ученики", payload={"cmd": f"tutor_students_{tid}"}))
        kb.row()
        kb.add(Callback("⚙️ Настроить расписание", payload={"cmd": f"tutor_schedule_{tid}"}))
        kb.row()
        kb.add(Callback("📊 Статистика", payload={"cmd": f"tutor_stats_{tid}"}))
        kb.row()
        kb.add(Callback("🔙 Назад в меню", payload={"cmd": "back_to_menu"}))
        await edit_event_message(event, "Панель преподавателя:", keyboard=kb.get_json())
    else:
        await edit_event_message(event, "Главное меню:", keyboard=await get_main_menu(event.user_id))
...
async def tutor_send_message_to_student(message: Message):
    ...
    try:
        await bot.api.messages.send(user_id=student_id, message=forward_msg, )
...
async def support_start(message: Message):
    kb = Keyboard(inline=True)
    kb.add(Callback("❌ Отмена", payload={"cmd": "cancel_support"}))
...
async def cancel_support(event: MessageEvent):
    await state_dispenser.delete(event.user_id)
    await edit_event_message(event, "Обращение отменено.")
    await bot.api.messages.send(user_id=event.user_id, message="Главное меню",
                                keyboard=await get_main_menu(event.user_id), )
...
async def support_message_to_admin(message: Message):
    ...
    kb = Keyboard(inline=True)
    kb.add(Callback("↩️ Ответить", payload={"cmd": f"support_reply_{uid}"}))
    try:
        await bot.api.messages.send(user_id=ADMIN_VK_ID, message=forward_msg, keyboard=kb.get_json(), )
    except:
        pass
...
async def support_reply_start(event: MessageEvent):
    ...
    await bot.api.messages.send(user_id=event.user_id, message="Введите ответ пользователю:", )
...
async def admin_panel(message: Message):
    if message.from_id != ADMIN_VK_ID:
        ...
        return
    kb = Keyboard(inline=True)
    kb.add(Callback("➕ Добавить репетитора", payload={"cmd": "admin_add"}))
    kb.row()
    kb.add(Callback("✏️ Редактировать репетитора", payload={"cmd": "admin_edit_list"}))
    kb.row()
    kb.add(Callback("❌ Удалить репетитора", payload={"cmd": "admin_delete_list"}))
    kb.row()
...
async def open_admin_panel(event: MessageEvent):
    ...
    kb.row()
    kb.add(Callback("🔙 Назад в меню", payload={"cmd": "back_to_menu"}))
    await edit_event_message(event, "Админ-панель управления репетиторами", keyboard=kb.get_json())
...
async def admin_add_start(event: MessageEvent):
    ...
    await edit_event_message(event, "Введите имя репетитора:")


@bot.on.private_message(state=AdminStates.waiting_name)
async def admin_add_name(message: Message):
    await state_dispenser.update(message.from_id, name=message.text.strip())
...
async def admin_add_description(message: Message):
    ...
    if desc == "-":
        desc = ""
    await state_dispenser.update(message.from_id, description=desc)
...
async def admin_add_telegram_id(message: Message):
    try:
        ...
        return
    await state_dispenser.update(message.from_id, telegram_id=tg_id if tg_id != 0 else None)
...
async def admin_add_vk_id(message: Message):
    try:
        ...
        await message.answer("Введите целое число или 0.")
        return
    await state_dispenser.update(message.from_id, vk_id=vk_id if vk_id != 0 else None)
...
async def admin_add_commission(message: Message):
    try:
        ...
        return
    await state_dispenser.update(message.from_id, commission_percent=comm)
...
async def admin_add_inn(message: Message):
    ...
    if inn == "-":
        inn = ""
    await state_dispenser.update(message.from_id, inn=inn)
    await state_dispenser.update(message.from_id, subjects={})
...
async def admin_add_subject_name(message: Message):
    ...
    await state_dispenser.update(message.from_id, temp_subject=subject)
...
async def admin_add_subject_price(message: Message):
    ...
    await state_dispenser.update(message.from_id, subjects=subjects)

    kb = Keyboard(inline=True)
    kb.add(Callback("✅ Да, добавить ещё", payload={"cmd": "add_another_subject"}))
    kb.row()
    kb.add(Callback("❌ Нет, закончить", payload={"cmd": "finish_adding_subjects"}))
...
async def add_another_subject(event: MessageEvent):
    await edit_event_message(event, "Введите название следующего предмета:")
...
async def finish_adding_subjects(event: MessageEvent):
    ...
    kb = Keyboard(inline=True)
    kb.add(Callback("📂 В админ-панель", payload={"cmd": "admin_panel_open"}))
    await edit_event_message(event, f"✅ Репетитор «{data['name']}» успешно добавлен (ID {new_id}).",
                             keyboard=kb.get_json())
    await state_dispenser.delete(event.user_id)
...
async def admin_edit_list(event: MessageEvent):
    ...
    if not tutors:
        kb = Keyboard(inline=True)
        kb.add(Callback("📂 В админ-панель", payload={"cmd": "admin_panel_open"}))
        await edit_event_message(event, "Нет репетиторов для редактирования.", keyboard=kb.get_json())
        return
    kb = Keyboard(inline=True)
    for tid, tdata in tutors.items():
        kb.add(Callback(tdata["name"], payload={"cmd": f"edit_tutor_{tid}"}))
        kb.row()
    kb.add(Callback("📂 В админ-панель", payload={"cmd": "admin_panel_open"}))
    await edit_event_message(event, "Выберите репетитора для редактирования:", keyboard=kb.get_json())


async def edit_tutor_choice(event: MessageEvent):
    tid = int(event.payload["cmd"].split("_")[-1])
    await state_dispenser.update(event.user_id, edit_tutor_id=tid)
    tutors = await get_all_tutors()
    tutor = tutors[tid]
    info = f"Редактирование: {tutor['name']}\n\nЧто хотите изменить?"
    kb = Keyboard(inline=True)
    kb.add(Callback("Изменить имя", payload={"cmd": "edit_name"}))
    kb.row()
    kb.add(Callback("Изменить описание", payload={"cmd": "edit_desc"}))
    kb.row()
    kb.add(Callback("Изменить VK ID", payload={"cmd": "edit_vk_id"}))
    kb.row()
    kb.add(Callback("📚 Управление предметами", payload={"cmd": "manage_subjects"}))
    kb.row()
    kb.add(Callback("💰 Изменить комиссию", payload={"cmd": "edit_commission"}))
    kb.row()
    kb.add(Callback("🔄 Режим комиссии", payload={"cmd": "toggle_commission_mode"}))
    kb.row()
    kb.add(Callback("🔙 К списку", payload={"cmd": "admin_edit_list"}))
    await edit_event_message(event, info, keyboard=kb.get_json())
...
async def edit_field_choice(event: MessageEvent):
    ...
    await state_dispenser.update(event.user_id, edit_field=field)
    prompts = {
...
async def manage_subjects(event: MessageEvent):
    ...
    if not tid or tid not in tutors:
        await answer_event(event, "Ошибка", snackbar=True)
        return
    tutor = tutors[tid]
    ...
    kb = Keyboard(inline=True)
    for subj in tutor["subjects"]:
        kb.add(Callback(f"✏️ {subj}", payload={"cmd": f"editsubj_{subj}"}))
        kb.row()
    kb.add(Callback("➕ Добавить предмет", payload={"cmd": "add_subject"}))
    kb.row()
    kb.add(Callback("🔙 Назад к редактированию", payload={"cmd": "back_to_edit_tutor"}))
    await edit_event_message(event, text, keyboard=kb.get_json())
...
async def add_subject_start(event: MessageEvent):
    await edit_event_message(event, "Введите название нового предмета:")
...
async def process_adding_subject_name(message: Message):
    ...
    if tid and name in tutors[tid]["subjects"]:
        ...
        return
    await state_dispenser.update(message.from_id, temp_new_subject=name)
...
async def edit_subject_menu(event: MessageEvent):
    ...
    await state_dispenser.update(event.user_id, edit_subject_name=subj_name)
    kb = Keyboard(inline=True)
    kb.add(Callback("✏️ Изменить название", payload={"cmd": "editsubj_name"}))
    kb.row()
    kb.add(Callback("💰 Изменить цену", payload={"cmd": "editsubj_price"}))
    kb.row()
    kb.add(Callback("❌ Удалить предмет", payload={"cmd": "editsubj_delete"}))
    kb.row()
    kb.add(Callback("🔙 Назад к списку предметов", payload={"cmd": "back_to_subjects_list"}))
    await edit_event_message(event, f"Предмет: {subj_name}\nВыберите действие:", keyboard=kb.get_json())
...
async def edit_subject_name_start(event: MessageEvent):
    await edit_event_message(event, "Введите новое название предмета:")
...
async def process_new_subject_name(message: Message):
    ...
    await message.answer(text, keyboard=kb.get_json())
    await state_dispenser.set(message.from_id, AdminStates.managing_subjects)
    await state_dispenser.update(message.from_id, edit_subject_name=None)


async def edit_subject_price_start(event: MessageEvent):
    await edit_event_message(event, "Введите новую цену (целое число):")
...
async def delete_subject_confirm(event: MessageEvent):
    ...
    kb.row()
    kb.add(Callback("❌ Отмена", payload={"cmd": "back_to_subjects_list"}))
    await edit_event_message(event, f"Удалить предмет «{subj}»?", keyboard=kb.get_json())
...
async def confirm_delete_subject(event: MessageEvent):
    ...
    await edit_event_message(event, f"✅ Предмет «{subj}» удалён.")
    tutors = await get_all_tutors()
    tutor = tutors[tid]
    text = f"Предметы репетитора «{tutor['name']}»:\n"
    for subj, price in tutor["subjects"].items():
...
async def admin_delete_list(event: MessageEvent):
    ...
    if not tutors:
        kb = Keyboard(inline=True)
        kb.add(Callback("📂 В админ-панель", payload={"cmd": "admin_panel_open"}))
        await edit_event_message(event, "Нет репетиторов для удаления.", keyboard=kb.get_json())
        return
    kb = Keyboard(inline=True)
    for tid, tdata in tutors.items():
        kb.add(Callback(tdata["name"], payload={"cmd": f"del_tutor_{tid}"}))
        kb.row()
    kb.add(Callback("📂 В админ-панель", payload={"cmd": "admin_panel_open"}))
    await edit_event_message(event, "Выберите репетитора для удаления:", keyboard=kb.get_json())
...
async def delete_tutor_confirm(event: MessageEvent):
    ...
    tutors = await get_all_tutors()
    tutor = tutors[tid]
    await state_dispenser.update(event.user_id, del_tutor_id=tid)
    kb = Keyboard(inline=True)
    kb.add(Callback("✅ Да, удалить", payload={"cmd": "confirm_delete"}))
    kb.row()
    kb.add(Callback("❌ Отмена", payload={"cmd": "admin_delete_list"}))
    await edit_event_message(event, f"Удалить репетитора «{tutor['name']}»?", keyboard=kb.get_json())
...
async def confirm_delete(event: MessageEvent):
    ...
    kb = Keyboard(inline=True)
    kb.add(Callback("📂 В админ-панель", payload={"cmd": "admin_panel_open"}))
    await edit_event_message(event, f"✅ Репетитор «{name}» удалён.", keyboard=kb.get_json())
    await state_dispenser.delete(event.user_id)
...
async def admin_stats_menu(event: MessageEvent):
    kb = Keyboard(inline=True)
    kb.add(Callback("👨‍🏫 Статистика по репетиторам", payload={"cmd": "admin_stats_tutors"}))
    kb.row()
    kb.add(Callback("👤 Статистика по ученикам", payload={"cmd": "admin_stats_students"}))
    kb.row()
    kb.add(Callback("🔙 В админ-панель", payload={"cmd": "admin_panel_open"}))
    await edit_event_message(event, "📊 Административная статистика\nВыберите раздел:", keyboard=kb.get_json())
...
async def admin_stats_tutors_month(event: MessageEvent):
    ...
    text = "\n".join(lines)
    kb = Keyboard(inline=True)
    kb.add(Callback("🔙 К общей статистике", payload={"cmd": "admin_stats_tutors"}))
    await edit_event_message(event, text, keyboard=kb.get_json())
...
async def tutor_panel(message: Message):
    ...
    if not tutor_id:
        ...
        return
    kb = Keyboard(inline=True)
    kb.add(Callback("👤 Моя анкета", payload={"cmd": f"tutor_profile_{tutor_id}"}))
    kb.row()
    kb.add(Callback("📋 Мои ученики", payload={"cmd": f"tutor_students_{tutor_id}"}))
    kb.row()
    kb.add(Callback("⚙️ Настроить расписание", payload={"cmd": f"tutor_schedule_{tutor_id}"}))
    kb.row()
    kb.add(Callback("📊 Статистика", payload={"cmd": f"tutor_stats_{tutor_id}"}))
    kb.row()
    kb.add(Callback("🔙 Назад в меню", payload={"cmd": "back_to_menu"}))
...
async def back_to_tutor_panel(event: MessageEvent):
    tid = int(event.payload["cmd"].split("_")[-1])
    kb = Keyboard(inline=True)
    kb.add(Callback("👤 Моя анкета", payload={"cmd": f"tutor_profile_{tid}"}))
    kb.row()
    kb.add(Callback("📋 Мои ученики", payload={"cmd": f"tutor_students_{tid}"}))
    kb.row()
    kb.add(Callback("⚙️ Настроить расписание", payload={"cmd": f"tutor_schedule_{tid}"}))
...
async def show_tutor_own_profile(event: MessageEvent):
    ...
    tutor = tutors.get(tid)
    if not tutor:
        await edit_event_message(event, "Анкета не найдена.")
        return
    text = tutor["description"] + "\n\nПредметы и цены:\n"
    for subj, price in tutor["subjects"].items():
        text += f"• {subj} — {price} руб.\n"
    kb = Keyboard(inline=True)
    kb.add(Callback("🔙 Назад в панель", payload={"cmd": f"back_to_tutor_panel_{tid}"}))
    await edit_event_message(event, text, keyboard=kb.get_json())
    ...
    for uid, sdata in students.items():
        ...
        for bid, b in sdata["bookings"]:
            ...
            if b["status"] == "pending":
                ...
                kb.add(Callback(f"❌ Отклонить", payload={"cmd": f"tutor_reject_{bid}"}))
                kb.row()
...
async def tutor_confirm_booking(event: MessageEvent):
    ...
    if email:
        ...
        await edit_event_message(event, "Заявка подтверждена. Отправлена ссылка на оплату.")
    else:
...
async def tutor_reject_booking(event: MessageEvent):
    ...
    await edit_event_message(event, "❌ Заявка отклонена.", keyboard=kb.get_json())
...
async def tutor_cancel_booking(event: MessageEvent):
    ...
    await bot.api.messages.send(user_id=student_id, message=msg, )
    tid = booking["tutor_id"]
    kb = Keyboard(inline=True)
    kb.add(Callback("🔙 К списку учеников", payload={"cmd": f"tutor_students_{tid}"}))
    await edit_event_message(event, "✅ Занятие отменено.", keyboard=kb.get_json())
...
async def tutor_reschedule_date(event: MessageEvent):
    ...
    kb = Keyboard(inline=True)
    for s in slots:
        kb.add(Callback(s, payload={"cmd": f"t_reschedule_slot_{s}"}))
        kb.row()
    kb.add(Callback("🔙 К выбору даты", payload={"cmd": "back_tutor_reschedule_date"}))
    await edit_event_message(event, "Выберите новое время:", keyboard=kb.get_json())
...
async def tutor_reschedule_slot(event: MessageEvent):
    ...
    kb = Keyboard(inline=True)
    kb.add(Callback("✅ Подтвердить", payload={"cmd": "confirm_tutor_reschedule"}))
    kb.row()
    kb.add(Callback("🔙 Назад", payload={"cmd": "back_tutor_reschedule_date"}))
    await edit_event_message(event, text, keyboard=kb.get_json())
...
async def confirm_tutor_reschedule(event: MessageEvent):
    ...
    await bot.api.messages.send(user_id=student_id, message=student_msg, )

    tutor_msg = f"✅ Вы перенесли занятие с {student_username} на {new_date} {new_time}."
    await bot.api.messages.send(user_id=event.user_id, message=tutor_msg, )

    kb = Keyboard(inline=True)
    kb.add(Callback("🔙 К списку учеников", payload={"cmd": f"tutor_students_{tid}"}))
    await edit_event_message(event, "Перенос выполнен.", keyboard=kb.get_json())
    await state_dispenser.delete(event.user_id)
...
async def schedule_main(event: MessageEvent):
    ...
    kb = Keyboard(inline=True)
    for day in WEEKDAYS:
        kb.add(Callback(f"✏️ {WEEKDAY_NAMES[day]}", payload={"cmd": f"sched_day_{day}"}))
        kb.row()
    kb.add(Callback("🔙 Назад", payload={"cmd": f"back_to_tutor_panel_{tid}"}))
    await edit_event_message(event, text, keyboard=kb.get_json())
...
async def edit_day(event: MessageEvent):
    ...
    kb = Keyboard(inline=True)
    kb.add(Callback("➕ Добавить слот", payload={"cmd": "add_slot"}))
    kb.row()
    kb.add(Callback("📅 Заполнить промежуток", payload={"cmd": "add_range"}))
    kb.row()
    if slots:
        kb.add(Callback("❌ Удалить слот", payload={"cmd": "del_slot"}))
        kb.row()
    if blocked:
        kb.add(Callback("🔓 Разблокировать день", payload={"cmd": f"unblock_day_{day}"}))
    else:
        kb.add(Callback("🔒 Заблокировать день", payload={"cmd": f"block_day_{day}"}))
    kb.row()
    kb.add(Callback("🔙 К дням недели", payload={"cmd": "back_to_schedule"}))
    await edit_event_message(event, text, keyboard=kb.get_json())
...
async def add_slot_start(event: MessageEvent):
    await edit_event_message(event, "Введите временной слот в формате HH:MM-HH:MM, например 10:00-11:30:")
...
async def add_range_start(event: MessageEvent):
    kb = Keyboard(inline=True)
    kb.add(Callback("60 минут", payload={"cmd": "dur_60"}))
    kb.row()
...
async def range_duration_chosen(event: MessageEvent):
    ...
    await state_dispenser.update(event.user_id, range_duration=duration)
    kb = Keyboard(inline=True)
    kb.add(Callback("Без перерыва", payload={"cmd": "brk_0"}))
    kb.row()
    kb.add(Callback("10 минут", payload={"cmd": "brk_10"}))
    kb.row()
    kb.add(Callback("15 минут", payload={"cmd": "brk_15"}))
    kb.row()
    kb.add(Callback("20 минут", payload={"cmd": "brk_20"}))
    kb.row()
    kb.add(Callback("🔙 Назад", payload={"cmd": "add_range_back"}))
    await edit_event_message(event, "Нужен ли перерыв между занятиями?", keyboard=kb.get_json())
...
async def range_break_chosen(event: MessageEvent):
    ...
    await state_dispenser.update(event.user_id, range_break=break_min)
    await edit_event_message(event,
...
async def del_slot_start(event: MessageEvent):
    ...
    slots = sched.get(day, [])
    if not slots:
        await edit_event_message(event, "Нет слотов для удаления.")
        return
    kb = Keyboard(inline=True)
    for s in slots:
...
async def confirm_del_slot(event: MessageEvent):
    ...
    await edit_event_message(event, "Слот удалён.")
    sched = await get_schedule(tid)
    slots = sched.get(day, [])
    text = f"Слоты для {WEEKDAY_NAMES[day]}:\n" + "\n".join(f"• {s}" for s in slots) if slots else "Нет слотов."
    kb = Keyboard(inline=True)
...
async def tutor_stats_month(event: MessageEvent):
    ...
    kb = Keyboard(inline=True)
    kb.add(Callback("🔙 К общей статистике", payload={"cmd": f"tutor_stats_{tid}"}))
    await edit_event_message(event, text, keyboard=kb.get_json())
    ...
    for bid, b in bookings.items():
        ...
        if timedelta(minutes=59) < diff <= timedelta(hours=1):
            ...
            if tutor and tutor.get("vk_id"):
                ...
                try:
                    await bot.api.messages.send(user_id=tutor["vk_id"], message=tutor_msg, )
                except:
                    pass
...
async def send_pending_reminders():
    ...
    for tid, plist in pending_by_tutor.items():
        ...
        try:
            await bot.api.messages.send(user_id=tutor["vk_id"], message=text, keyboard=keyboard.get_json(), )
